...
if (-not $env:BEACON_API_KEY) { exit 0 }
$baseUrl = if ($env:BEACON_URL) { $env:BEACON_URL } else { 'https://beacon-tool.vercel.app' }
...
try {
  Invoke-RestMethod -Method Post -Uri "$baseUrl/api/events" -TimeoutSec 5 `
    -Headers @{ Authorization = "Bearer $($env:BEACON_API_KEY)" } `
