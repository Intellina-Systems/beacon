...
export function SignIn() {
  ...
  const busy = loadingVercel || loadingGitHub
  ...
  return (
    <div className="flex w-full max-w-xs flex-col gap-3">
      {hasGitHub && (
        <Button onClick={handleGitHubSignIn} disabled={busy} size="lg" className="w-full">
          {loadingGitHub ? <Spinner /> : <GitHubIcon />}
      ...
      {hasVercel && (
        <Button
          ...
          disabled={busy}
          variant={hasGitHub ? 'outline' : 'default'}
          size="lg"
          className="w-full"
        >
          {loadingVercel ? <Spinner /> : <VercelIcon />}
          {loadingVercel ? 'Redirecting…' : 'Sign in with Vercel'}
        </Button>
      )}
    </div>
  )
