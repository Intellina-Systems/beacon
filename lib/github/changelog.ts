...
export class ChangelogAiConfigurationError extends Error {}
export class GitHubNotConnectedError extends Error {}

const COMMIT_SUMMARY_MODEL = 'gpt-5.4-nano'

const getCommitSummaryModel = () => {
  if (!process.env.OPENAI_API_KEY) {
    throw new ChangelogAiConfigurationError('OpenAI API key not configured')
  }

  return openai(COMMIT_SUMMARY_MODEL)
}
...
export async function generateProductChangelog(
  userId: string,
  productId: string,
  since: Date,
): Promise<{ range: { from: string; to: string }; entries: ChangelogEntry[] }> {
  const token = await getGitHubTokenForUserId(userId)
  if (!token) {
    throw new GitHubNotConnectedError('GitHub account not connected')
  }
  ...
  const repositories = await db
    ...
    .where(eq(productGitHubRepositories.productId, productId))
  ...
  const pullRequests = await db
    ...
    .where(and(eq(githubPullRequests.productId, productId), gte(githubPullRequests.githubUpdatedAt, since)))
  ...
  const prDataWithFiles = await mapWithConcurrency(pullRequests, 5, async (pr) => {
    ...
    const fileSummary: ChangelogFileSummary = { added: [], removed: [], modified: [], renamed: [] }
    ...
    return { pr, files: fileSummary, additions, deletions }
  })
  ...
  const entries: ChangelogEntry[] = validatedData.map((item) => ({
    number: item.pr.number,
    title: item.pr.title,
  ...
  return { range: { from: since.toISOString(), to: toDate.toISOString() }, entries }
}

export async function generateCommitsSummaryText(
  commits: Array<{ message: string; authorLogin?: string | null; committedAt?: Date | null }>,
): Promise<string> {
